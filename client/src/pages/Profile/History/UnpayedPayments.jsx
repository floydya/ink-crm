...
import { Button, Descriptions, Popconfirm, Table, message } from "antd";
...
const PaymentActions = ({ entity }) => {
  ...
  const cancelEntity = useCallback(
    async () => {
      const reason = await prompt({
        ...
        rules: [
          { required: true, message: "Это поле обязательное для заполнения!" },
        ],
      });
      ...
      if (reason) {
        ...
        message.info("Платеж успешно отменен!");
  ...
  return (
    <>
      <Popconfirm
        title="Вы уверены что хотите подтвердить платеж?"
        onConfirm={applyEntity}
        okText="Да"
        cancelText="Нет"
      >
        <Button type="link">Подтвердить</Button>
      </Popconfirm>
      <Button type="link" onClick={cancelEntity}>
        Отменить
      </Button>
    </>
  );
...
const UnpayedPayments = () => {
  ...
  const [{ data: bounties }, fetchBounties] = useApi.get(
    `/bounties/`,
    {
  ...
  const [{ data: fines }, fetchFines] = useApi.get(
    `/fines/`,
    {
      employee_id: profile.id,
      status: "null",
  ...
  const data = useMemo(
    () => {
      const _b = (bounties || []).map((el) => ({
        ...el,
        key: `${el.id}__bounty`,
        __type: "bounty",
        color: "rgba(0, 255, 0, 0.05)",
      }));
      const _f = (fines || []).map((el) => ({
        ...el,
        key: `${el.id}__fine`,
        __type: "fine",
        color: "rgba(255, 0, 0, 0.05)",
      }));
      return [..._b, ..._f].sort((a, b) =>
  ...
  return (
    <Table
      title={() => <h2>Невыплаченные платежи</h2>}
      columns={columns}
      dataSource={data}
      expandable={{
