...
const AntForm = ({ _form, handleSubmit, formProps, children }) => {
  ...
  const onFinish = React.useCallback(
    async (values) => {
      try {
        ...
        if (errors?.data) {
          const formData = Object.entries(errors.data).map(([name, errors]) => ({ name, errors }))
  ...
  console.log(form.getFieldsError(["non_field_errors", "detail"]))
  return <Form {...formProps} form={form} onFinish={onFinish}>
